...
Additionally, connection information for an s3-compatible store can be passed to enable remote snapshots.
...
- **s3_endpoint**: Endpoint of the s3 store that will be used for snapshots. Can be left empty in which case with will default to **aws**.
- **s3_protocol**: Protocol to access the s3 endpoint for snapshots. Defaults to **https**.
- **s3_access_key**: Access key to authenticate against the s3 store where snapshots will go. If left empty, there won't be any s3 configurations for snapshots.
- **s3_secret_key**: Secret key to authenticate against the s3 store where snapshots will go.
